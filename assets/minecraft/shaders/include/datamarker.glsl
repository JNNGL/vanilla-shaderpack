...
bool discardDataMarker(ivec2 pixel) {
    return pixel.y >= 1 || pixel.x > 31;
}

vec4 writeDataMarker(ivec2 pixel, mat4 projMat, float fogStart, float fogEnd, vec3 chunkOffset, float gameTime, bool isShadowMap, mat3 viewMat) {
...
bool overlayShadowMap(vec2 fragCoord, inout vec4 color, vec3 shadowOffset) {
